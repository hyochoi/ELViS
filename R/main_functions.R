...
get_outmat = function(Y,sam) {
  ...
  outmat = t(apply(win,1,function(x) pd.rate.hy(apply(Y[c(x[1]:x[2]),],2,mean),qrsc = T)))
  return(outmat)
}
...
get_normalized_data = function(X,q.cutoff=4){
  ...
  window.list = sapply(c(1:ncol(Y)),function(j) get_window_v1(Y=Y,sam=j)) # get windows
  ...
  for (sam in seq_len(n) ) {
    ...
    Y2[,sam] = Y[,sam]/median(Y[which(! c(1:d) %in% outreg),sam])
  }

  Z2 = t(apply(Y2,1,function(x) pd.rate.hy(x,qrsc=T))) # stabilize deviations seperately for higher than median and lower than median

  # final normalized data
  output = list(sample_Ids=colnames(X),Y=Y2,Z=Z2)

  return(output)
  # window.list2 = sapply(c(1:ncol(Y2)),function(j) get_window_v1(Y=Y2,sam=j))
  # # outmat.list2 = sapply(c(1:ncol(Y2)),function(j) get_outmat(Y=Y2,sam=j))
...
update_reference_segments = function(normalized_data,cores=10L){
      ...
      mclapply(
        ...
        function(sam) {
          ...
          output = tryCatch({
            ...
            cat(paste(sam,"|",err),"\n")
            print("Running 1D segmentation with robust-scaled data only")
  ...
  # system.time({
    ...
    Z_recentered = t(apply(Y_recentered,1,function(x) pd.rate.hy(x,qrsc=T)))
  # })
...
finalize_segments_and_clusters = function(segupdated_data){
  ...
  rescued_data =
    segupdated_data %>%
    within({
      for (sam in which(segment.K==0)) {
        print(sam)
        sampleID = sample_Ids[sam]
        testdata = data.frame(z=Z[,sam],y=Y[,sam])
...
detect_bp__update_ref = function(
  ...
  # time1 = system.time({
    message("BPcut1")
    BPcut1 <- sapply(1:dim(X)[2], FUN=function(sam) which(abs(Xdiff[,sam]) > X.cutoff))
  # })
  # print(time1)
  # time2 = system.time({
    message("BPcut2")
    BPcut2 <- sapply(1:dim(X)[2], FUN=function(sam) which(abs(Ydiff[,sam]) > BP.ydepth))
  # })
  # print(time2)
  # time21 = system.time({
    message("BPs2.level1")
    BPs2.level1 <- sapply(1:dim(X)[2], FUN=function(sam) base::intersect(BPs2.test05[[sam]], BPcut1[[sam]]))
  # })
  # print(time21)
  # time22 = system.time({
    message("BPs2.level2")
    BPs2.level2 <- sapply(1:dim(X)[2], FUN=function(sam) base::intersect(BPs2.test10[[sam]], BPcut1[[sam]]))
  # })
  # print(time22)
  # time21p = system.time({
    message("BPs2.level1.plus")
    BPs2.level1.plus <- sapply(1:dim(X)[2], FUN=function(sam) base::intersect(BPs2.level1[[sam]], BPcut2[[sam]]))
  # })
  # print(time21p)
    ...
    message("Y_new")
      Y_new = (1:ncol(X)) %>%
        ...
        mclapply(mc.cores = cores,
          ...
          \(i) {print(i);detect_bp__update_ref__sub_updateY(i,X_input=X,Y=Y,Z=Z,BPs2.level1.plus=BPs2.level1.plus,BPs2.level2=BPs2.level2)}
        ) %>%
        as.data.frame() %>% as.matrix() %>% magrittr::set_names(NULL)
    # })
  Z_new =  t(apply(Y_new,1,function(x) pd.rate.hy(x,qrsc=T))) ###############################################
  ...
  message("Y_new2")
      Y_new2 = (1:ncol(X)) %>%
        ...
        as.data.frame() %>% as.matrix() %>% magrittr::set_names(NULL)
    # })
  Z_new2 =  t(apply(Y_new2,1,function(x) pd.rate.hy(x,qrsc=T))) ###############################################
  # print(time_test)
  # Y_new = test %>% as.data.frame() %>% as.matrix() %>% magrittr::set_names(NULL)
  # print(time_test)
  # user   system  elapsed
...
#' @param verbose logical indicating whether to print out messages for debugging
#'
    ...
    if(overwrite|(!file.exists(rds_file))){
      refupate_data_secondrun =
        update_reference_segments(
          refupate_data %>%
            with(list(
              sample_Ids=sample_Ids,Y=Y_recentered,Z=Z_recentered
            )),
          cores = N_cores
        )
    ...
    return(
      list(
        ...
        final_call =
          ...
          dplyr::summarise(N_seg = n(),N_state = length(unique(state))) %>%
          with(list(segmented_samples = which(N_seg!=1), cnv_samples = which(N_state!=1))),
...
get_new_baseline = function(result,mode="longest"){
  ...
  tmp_data =
    ...
    dplyr::summarise(total_length = sum(end-begin+1)) %>%
    dplyr::ungroup()
  ...
  if(mode == "longest"){
    ...
    new_baseline =
      tmp_data %>%
      dplyr::group_by(id) %>%
      dplyr::slice_max(total_length,n = 1) %>%
      with(state)
    ...
    new_baseline =
      tmp_data %>%
      dplyr::group_by(id) %>%
      dplyr::slice_min(total_length,n = 1) %>%
      with(state)
